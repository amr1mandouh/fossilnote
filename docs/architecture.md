# FossilNote architecture

FossilNote is a small, client-only React application. `useKilnWhisper` owns the active load, filters, and selection set. Components receive state and callbacks, while `lib/filters.ts` keeps filtering deterministic and easy to test.
