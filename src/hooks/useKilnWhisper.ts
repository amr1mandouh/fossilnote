...
export function useKilnWhisper() {
  ...
  const switchLoad = useCallback((id: string) => {
    ...
    setSelected(new Set())
    setAnnouncement('Collection changed and filters reset.')
  }, [])
  ...
  const markSelected = useCallback((cataloged: boolean) => {
    setLoads((current) => current.map((load) => load.id !== activeLoad?.id ? load : ({ ...load, pieces: load.pieces.map((piece) => selected.has(piece.id) ? { ...piece, cataloged } : piece) })))
    setAnnouncement(selected.size + ' selected specimen' + (selected.size === 1 ? '' : 's') + ' marked ' + (cataloged ? 'cataloged' : 'waiting') + '.')
    setSelected(new Set())
  ...
  const addPiece = useCallback((piece: Omit<Piece, 'id' | 'number'>) => {
    ...
    setAnnouncement(piece.name + ' added to the ')
  }, [activeLoad?.id])
...
export type FossilNoteState = ReturnType<typeof useKilnWhisper>
